...
private fun PlansTab(
    ...
    // Preset планы из Room, разбитые по уровню
    ...
    LazyColumn(
        ...
        if (beginnerPresets.isNotEmpty()) {
            item {
                PlanSectionHeader("Новичок", "Базовые программы · 3–4 дня/нед", "🌱", Color(0xFF4CAF50))
            }
            item {
                PresetPlanRow(
        ...
        if (intermediatePresets.isNotEmpty()) {
            item {
                PlanSectionHeader("Средний", "Сплит-программы · 4–5 дней/нед", "💎", Color(0xFF2196F3))
            }
            item {
                PresetPlanRow(
        ...
        if (advancedPresets.isNotEmpty()) {
            item {
                PlanSectionHeader("Опытный", "PPL и силовые · 5–6 дней/нед", "⚡", Color(0xFFFF9800))
            }
            item {
                PresetPlanRow(
        ...
        // ── Мои планы ─────────────────────────────────────────────
        item {
            PlanSectionHeader("Мои планы", "Созданные вами", "✏️", Color(0xFF7C4DFF))
        }
        item {
            UserPlansRow(
...
// ── Preset plan row ───────────────────────────────────────────────
// Теперь принимает WorkoutPlanEntity (из Room) — у каждого есть реальный id
...
private fun UserPlanCard(
    ...
    Box(
        ...
        Column(
            ...
            // Только Edit и Delete — кнопки «Выбрать» нет
