...
class CalculateStrengthLevelUseCase @Inject constructor(
    ...
    suspend operator fun invoke(): StrengthLevel {
        ...
        // ── Ранг по 1RM (используем best5RM как proxy, т.к. это лучший доступный показатель) ──
        val bench1Rm  = best5RM[BigThreeLift.BENCH_PRESS] ?: 0.0
        val squat1Rm  = best5RM[BigThreeLift.BACK_SQUAT]  ?: 0.0
        val dead1Rm   = best5RM[BigThreeLift.DEADLIFT]    ?: 0.0
        val rank = StrengthRanks.currentRankFor(bench1Rm, squat1Rm, dead1Rm)
    ...
    private fun computeProgressToNext(currentLevel: Int, best5RM: Map<BigThreeLift, Double>): Float {
    ...
    private fun computeKgToNext(currentLevel: Int, best5RM: Map<BigThreeLift, Double>): Map<BigThreeLift, Double> {
